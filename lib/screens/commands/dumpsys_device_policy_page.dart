...
class DumpsysDevicePolicyPage extends StatefulWidget {
  const DumpsysDevicePolicyPage({super.key});

  @override
  State<DumpsysDevicePolicyPage> createState() => _DumpsysDevicePolicyPageState();
}

class _DumpsysDevicePolicyPageState extends State<DumpsysDevicePolicyPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysDevicePolicyService.fetch(deviceId);
    if (mounted) {
      setState(() {
