...
class CmdWifiStatusPage extends StatefulWidget {
  const CmdWifiStatusPage({super.key});

  @override
  State<CmdWifiStatusPage> createState() => _CmdWifiStatusPageState();
}

class _CmdWifiStatusPageState extends State<CmdWifiStatusPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await CmdWifiStatusService.fetch(deviceId);
    if (mounted) {
      setState(() {
