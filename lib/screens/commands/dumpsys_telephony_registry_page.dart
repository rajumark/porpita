...
class DumpsysTelephonyRegistryPage extends StatefulWidget {
  const DumpsysTelephonyRegistryPage({super.key});

  @override
  State<DumpsysTelephonyRegistryPage> createState() => _DumpsysTelephonyRegistryPageState();
}

class _DumpsysTelephonyRegistryPageState extends State<DumpsysTelephonyRegistryPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysTelephonyRegistryService.fetch(deviceId);
    if (mounted) {
      setState(() {
