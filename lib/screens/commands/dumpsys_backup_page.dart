...
class DumpsysBackupPage extends StatefulWidget {
  const DumpsysBackupPage({super.key});

  @override
  State<DumpsysBackupPage> createState() => _DumpsysBackupPageState();
}

class _DumpsysBackupPageState extends State<DumpsysBackupPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysBackupService.fetch(deviceId);
    if (mounted) {
      setState(() {
