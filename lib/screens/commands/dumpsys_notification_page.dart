...
class DumpsysNotificationPage extends StatefulWidget {
  const DumpsysNotificationPage({super.key});

  @override
  State<DumpsysNotificationPage> createState() => _DumpsysNotificationPageState();
}

class _DumpsysNotificationPageState extends State<DumpsysNotificationPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysNotificationService.fetch(deviceId);
    if (mounted) {
      setState(() {
