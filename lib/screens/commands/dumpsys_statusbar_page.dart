...
class DumpsysStatusbarPage extends StatefulWidget {
  const DumpsysStatusbarPage({super.key});

  @override
  State<DumpsysStatusbarPage> createState() => _DumpsysStatusbarPageState();
}

class _DumpsysStatusbarPageState extends State<DumpsysStatusbarPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysStatusbarService.fetch(deviceId);
    if (mounted) {
      setState(() {
