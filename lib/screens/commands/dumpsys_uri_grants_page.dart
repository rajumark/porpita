...
class DumpsysUriGrantsPage extends StatefulWidget {
  const DumpsysUriGrantsPage({super.key});

  @override
  State<DumpsysUriGrantsPage> createState() => _DumpsysUriGrantsPageState();
}

class _DumpsysUriGrantsPageState extends State<DumpsysUriGrantsPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysUriGrantsService.fetch(deviceId);
    if (mounted) {
      setState(() {
