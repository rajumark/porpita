...
class DumpsysPage extends StatefulWidget {
  const DumpsysPage({super.key});

  @override
  State<DumpsysPage> createState() => _DumpsysPageState();
}

class _DumpsysPageState extends State<DumpsysPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysService.fetch(deviceId);
    if (mounted) {
      setState(() {
