...
class DumpsysActivityBroadcastsPage extends StatefulWidget {
  const DumpsysActivityBroadcastsPage({super.key});

  @override
  State<DumpsysActivityBroadcastsPage> createState() => _DumpsysActivityBroadcastsPageState();
}

class _DumpsysActivityBroadcastsPageState extends State<DumpsysActivityBroadcastsPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysActivityBroadcastsService.fetch(deviceId);
    if (mounted) {
      setState(() {
