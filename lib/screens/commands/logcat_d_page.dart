...
class LogcatDPage extends StatefulWidget {
  const LogcatDPage({super.key});

  @override
  State<LogcatDPage> createState() => _LogcatDPageState();
}

class _LogcatDPageState extends State<LogcatDPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await LogcatDService.fetch(deviceId);
    if (mounted) {
      setState(() {
