...
class ProcrankPage extends StatefulWidget {
  const ProcrankPage({super.key});

  @override
  State<ProcrankPage> createState() => _ProcrankPageState();
}

class _ProcrankPageState extends State<ProcrankPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await ProcrankService.fetch(deviceId);
    if (mounted) {
      setState(() {
