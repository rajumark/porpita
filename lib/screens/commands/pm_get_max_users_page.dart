...
class PmGetMaxUsersPage extends StatefulWidget {
  const PmGetMaxUsersPage({super.key});

  @override
  State<PmGetMaxUsersPage> createState() => _PmGetMaxUsersPageState();
}

class _PmGetMaxUsersPageState extends State<PmGetMaxUsersPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await PmGetMaxUsersService.fetch(deviceId);
    if (mounted) {
      setState(() {
