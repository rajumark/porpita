...
class DumpsysNetpolicyPage extends StatefulWidget {
  const DumpsysNetpolicyPage({super.key});

  @override
  State<DumpsysNetpolicyPage> createState() => _DumpsysNetpolicyPageState();
}

class _DumpsysNetpolicyPageState extends State<DumpsysNetpolicyPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysNetpolicyService.fetch(deviceId);
    if (mounted) {
      setState(() {
