...
class PmListInstrumentationPage extends StatefulWidget {
  const PmListInstrumentationPage({super.key});

  @override
  State<PmListInstrumentationPage> createState() => _PmListInstrumentationPageState();
}

class _PmListInstrumentationPageState extends State<PmListInstrumentationPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await PmListInstrumentationService.fetch(deviceId);
    if (mounted) {
      setState(() {
