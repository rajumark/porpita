...
class DumpsysActivityProcessesPage extends StatefulWidget {
  const DumpsysActivityProcessesPage({super.key});

  @override
  State<DumpsysActivityProcessesPage> createState() => _DumpsysActivityProcessesPageState();
}

class _DumpsysActivityProcessesPageState extends State<DumpsysActivityProcessesPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysActivityProcessesService.fetch(deviceId);
    if (mounted) {
      setState(() {
