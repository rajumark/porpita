...
class DumpsysBatterystatsPage extends StatefulWidget {
  const DumpsysBatterystatsPage({super.key});

  @override
  State<DumpsysBatterystatsPage> createState() => _DumpsysBatterystatsPageState();
}

class _DumpsysBatterystatsPageState extends State<DumpsysBatterystatsPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysBatterystatsService.fetch(deviceId);
    if (mounted) {
      setState(() {
