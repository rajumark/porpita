...
class DumpsysAccountPage extends StatefulWidget {
  const DumpsysAccountPage({super.key});

  @override
  State<DumpsysAccountPage> createState() => _DumpsysAccountPageState();
}

class _DumpsysAccountPageState extends State<DumpsysAccountPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysAccountService.fetch(deviceId);
    if (mounted) {
      setState(() {
