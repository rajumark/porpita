...
class DumpsysActivityPage extends StatefulWidget {
  const DumpsysActivityPage({super.key});

  @override
  State<DumpsysActivityPage> createState() => _DumpsysActivityPageState();
}

class _DumpsysActivityPageState extends State<DumpsysActivityPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysActivityService.fetch(deviceId);
    if (mounted) {
      setState(() {
