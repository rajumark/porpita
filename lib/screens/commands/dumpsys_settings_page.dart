...
class DumpsysSettingsPage extends StatefulWidget {
  const DumpsysSettingsPage({super.key});

  @override
  State<DumpsysSettingsPage> createState() => _DumpsysSettingsPageState();
}

class _DumpsysSettingsPageState extends State<DumpsysSettingsPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysSettingsService.fetch(deviceId);
    if (mounted) {
      setState(() {
