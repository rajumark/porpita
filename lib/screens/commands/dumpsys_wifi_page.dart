...
class DumpsysWifiPage extends StatefulWidget {
  const DumpsysWifiPage({super.key});

  @override
  State<DumpsysWifiPage> createState() => _DumpsysWifiPageState();
}

class _DumpsysWifiPageState extends State<DumpsysWifiPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysWifiService.fetch(deviceId);
    if (mounted) {
      setState(() {
