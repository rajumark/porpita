...
class DumpsysMountPage extends StatefulWidget {
  const DumpsysMountPage({super.key});

  @override
  State<DumpsysMountPage> createState() => _DumpsysMountPageState();
}

class _DumpsysMountPageState extends State<DumpsysMountPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysMountService.fetch(deviceId);
    if (mounted) {
      setState(() {
