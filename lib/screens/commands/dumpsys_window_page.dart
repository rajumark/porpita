...
class DumpsysWindowPage extends StatefulWidget {
  const DumpsysWindowPage({super.key});

  @override
  State<DumpsysWindowPage> createState() => _DumpsysWindowPageState();
}

class _DumpsysWindowPageState extends State<DumpsysWindowPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysWindowService.fetch(deviceId);
    if (mounted) {
      setState(() {
