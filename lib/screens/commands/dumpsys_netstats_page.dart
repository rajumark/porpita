...
class DumpsysNetstatsPage extends StatefulWidget {
  const DumpsysNetstatsPage({super.key});

  @override
  State<DumpsysNetstatsPage> createState() => _DumpsysNetstatsPageState();
}

class _DumpsysNetstatsPageState extends State<DumpsysNetstatsPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysNetstatsService.fetch(deviceId);
    if (mounted) {
      setState(() {
