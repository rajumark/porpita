...
class DumpsysActivityProvidersPage extends StatefulWidget {
  const DumpsysActivityProvidersPage({super.key});

  @override
  State<DumpsysActivityProvidersPage> createState() => _DumpsysActivityProvidersPageState();
}

class _DumpsysActivityProvidersPageState extends State<DumpsysActivityProvidersPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysActivityProvidersService.fetch(deviceId);
    if (mounted) {
      setState(() {
