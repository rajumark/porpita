...
class DumpsysCpuinfoPage extends StatefulWidget {
  const DumpsysCpuinfoPage({super.key});

  @override
  State<DumpsysCpuinfoPage> createState() => _DumpsysCpuinfoPageState();
}

class _DumpsysCpuinfoPageState extends State<DumpsysCpuinfoPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysCpuinfoService.fetch(deviceId);
    if (mounted) {
      setState(() {
