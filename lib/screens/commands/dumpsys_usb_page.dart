...
class DumpsysUsbPage extends StatefulWidget {
  const DumpsysUsbPage({super.key});

  @override
  State<DumpsysUsbPage> createState() => _DumpsysUsbPageState();
}

class _DumpsysUsbPageState extends State<DumpsysUsbPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysUsbService.fetch(deviceId);
    if (mounted) {
      setState(() {
