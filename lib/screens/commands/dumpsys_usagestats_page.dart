...
class DumpsysUsagestatsPage extends StatefulWidget {
  const DumpsysUsagestatsPage({super.key});

  @override
  State<DumpsysUsagestatsPage> createState() => _DumpsysUsagestatsPageState();
}

class _DumpsysUsagestatsPageState extends State<DumpsysUsagestatsPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysUsagestatsService.fetch(deviceId);
    if (mounted) {
      setState(() {
