...
class DumpsysAudioPage extends StatefulWidget {
  const DumpsysAudioPage({super.key});

  @override
  State<DumpsysAudioPage> createState() => _DumpsysAudioPageState();
}

class _DumpsysAudioPageState extends State<DumpsysAudioPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysAudioService.fetch(deviceId);
    if (mounted) {
      setState(() {
