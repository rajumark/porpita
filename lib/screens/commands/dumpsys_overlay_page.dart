...
class DumpsysOverlayPage extends StatefulWidget {
  const DumpsysOverlayPage({super.key});

  @override
  State<DumpsysOverlayPage> createState() => _DumpsysOverlayPageState();
}

class _DumpsysOverlayPageState extends State<DumpsysOverlayPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysOverlayService.fetch(deviceId);
    if (mounted) {
      setState(() {
