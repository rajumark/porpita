...
class DumpsysActivityRecentsPage extends StatefulWidget {
  const DumpsysActivityRecentsPage({super.key});

  @override
  State<DumpsysActivityRecentsPage> createState() => _DumpsysActivityRecentsPageState();
}

class _DumpsysActivityRecentsPageState extends State<DumpsysActivityRecentsPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysActivityRecentsService.fetch(deviceId);
    if (mounted) {
      setState(() {
