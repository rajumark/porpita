...
class AmDumpheapPage extends StatefulWidget {
  const AmDumpheapPage({super.key});

  @override
  State<AmDumpheapPage> createState() => _AmDumpheapPageState();
}

class _AmDumpheapPageState extends State<AmDumpheapPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await AmDumpheapService.fetch(deviceId);
    if (mounted) {
      setState(() {
