...
class DumpsysMediaSessionPage extends StatefulWidget {
  const DumpsysMediaSessionPage({super.key});

  @override
  State<DumpsysMediaSessionPage> createState() => _DumpsysMediaSessionPageState();
}

class _DumpsysMediaSessionPageState extends State<DumpsysMediaSessionPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysMediaSessionService.fetch(deviceId);
    if (mounted) {
      setState(() {
