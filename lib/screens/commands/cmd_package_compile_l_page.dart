...
class CmdPackageCompileLPage extends StatefulWidget {
  const CmdPackageCompileLPage({super.key});

  @override
  State<CmdPackageCompileLPage> createState() => _CmdPackageCompileLPageState();
}

class _CmdPackageCompileLPageState extends State<CmdPackageCompileLPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await CmdPackageCompileLService.fetch(deviceId);
    if (mounted) {
      setState(() {
