...
class DumpsysKeystorePage extends StatefulWidget {
  const DumpsysKeystorePage({super.key});

  @override
  State<DumpsysKeystorePage> createState() => _DumpsysKeystorePageState();
}

class _DumpsysKeystorePageState extends State<DumpsysKeystorePage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysKeystoreService.fetch(deviceId);
    if (mounted) {
      setState(() {
