...
class DumpsysNetworkManagementPage extends StatefulWidget {
  const DumpsysNetworkManagementPage({super.key});

  @override
  State<DumpsysNetworkManagementPage> createState() => _DumpsysNetworkManagementPageState();
}

class _DumpsysNetworkManagementPageState extends State<DumpsysNetworkManagementPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysNetworkManagementService.fetch(deviceId);
    if (mounted) {
      setState(() {
