...
class PmDumpPage extends StatefulWidget {
  const PmDumpPage({super.key});

  @override
  State<PmDumpPage> createState() => _PmDumpPageState();
}

class _PmDumpPageState extends State<PmDumpPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await PmDumpService.fetch(deviceId);
    if (mounted) {
      setState(() {
