...
class DumpsysBatteryPage extends StatefulWidget {
  const DumpsysBatteryPage({super.key});

  @override
  State<DumpsysBatteryPage> createState() => _DumpsysBatteryPageState();
}

class _DumpsysBatteryPageState extends State<DumpsysBatteryPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysBatteryService.fetch(deviceId);
    if (mounted) {
      setState(() {
