...
class PmListFeaturesPage extends StatefulWidget {
  const PmListFeaturesPage({super.key});

  @override
  State<PmListFeaturesPage> createState() => _PmListFeaturesPageState();
}

class _PmListFeaturesPageState extends State<PmListFeaturesPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await PmListFeaturesService.fetch(deviceId);
    if (mounted) {
      setState(() {
