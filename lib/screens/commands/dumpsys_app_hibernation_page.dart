...
class DumpsysAppHibernationPage extends StatefulWidget {
  const DumpsysAppHibernationPage({super.key});

  @override
  State<DumpsysAppHibernationPage> createState() => _DumpsysAppHibernationPageState();
}

class _DumpsysAppHibernationPageState extends State<DumpsysAppHibernationPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysAppHibernationService.fetch(deviceId);
    if (mounted) {
      setState(() {
