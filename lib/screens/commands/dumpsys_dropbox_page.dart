...
class DumpsysDropboxPage extends StatefulWidget {
  const DumpsysDropboxPage({super.key});

  @override
  State<DumpsysDropboxPage> createState() => _DumpsysDropboxPageState();
}

class _DumpsysDropboxPageState extends State<DumpsysDropboxPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysDropboxService.fetch(deviceId);
    if (mounted) {
      setState(() {
