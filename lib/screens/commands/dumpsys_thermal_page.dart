...
class DumpsysThermalPage extends StatefulWidget {
  const DumpsysThermalPage({super.key});

  @override
  State<DumpsysThermalPage> createState() => _DumpsysThermalPageState();
}

class _DumpsysThermalPageState extends State<DumpsysThermalPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysThermalService.fetch(deviceId);
    if (mounted) {
      setState(() {
