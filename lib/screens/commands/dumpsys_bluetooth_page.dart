...
class DumpsysBluetoothPage extends StatefulWidget {
  const DumpsysBluetoothPage({super.key});

  @override
  State<DumpsysBluetoothPage> createState() => _DumpsysBluetoothPageState();
}

class _DumpsysBluetoothPageState extends State<DumpsysBluetoothPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysBluetoothService.fetch(deviceId);
    if (mounted) {
      setState(() {
