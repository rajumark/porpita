...
class CmdOverlayListPage extends StatefulWidget {
  const CmdOverlayListPage({super.key});

  @override
  State<CmdOverlayListPage> createState() => _CmdOverlayListPageState();
}

class _CmdOverlayListPageState extends State<CmdOverlayListPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await CmdOverlayListService.fetch(deviceId);
    if (mounted) {
      setState(() {
