...
class DumpsysFingerprintPage extends StatefulWidget {
  const DumpsysFingerprintPage({super.key});

  @override
  State<DumpsysFingerprintPage> createState() => _DumpsysFingerprintPageState();
}

class _DumpsysFingerprintPageState extends State<DumpsysFingerprintPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysFingerprintService.fetch(deviceId);
    if (mounted) {
      setState(() {
