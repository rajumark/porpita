...
class DumpsysLocationPage extends StatefulWidget {
  const DumpsysLocationPage({super.key});

  @override
  State<DumpsysLocationPage> createState() => _DumpsysLocationPageState();
}

class _DumpsysLocationPageState extends State<DumpsysLocationPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysLocationService.fetch(deviceId);
    if (mounted) {
      setState(() {
