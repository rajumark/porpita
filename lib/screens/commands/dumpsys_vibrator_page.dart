...
class DumpsysVibratorPage extends StatefulWidget {
  const DumpsysVibratorPage({super.key});

  @override
  State<DumpsysVibratorPage> createState() => _DumpsysVibratorPageState();
}

class _DumpsysVibratorPageState extends State<DumpsysVibratorPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysVibratorService.fetch(deviceId);
    if (mounted) {
      setState(() {
