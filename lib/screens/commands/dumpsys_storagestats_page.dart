...
class DumpsysStoragestatsPage extends StatefulWidget {
  const DumpsysStoragestatsPage({super.key});

  @override
  State<DumpsysStoragestatsPage> createState() => _DumpsysStoragestatsPageState();
}

class _DumpsysStoragestatsPageState extends State<DumpsysStoragestatsPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysStoragestatsService.fetch(deviceId);
    if (mounted) {
      setState(() {
