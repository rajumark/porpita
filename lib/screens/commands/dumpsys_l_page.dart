...
class DumpsysLPage extends StatefulWidget {
  const DumpsysLPage({super.key});

  @override
  State<DumpsysLPage> createState() => _DumpsysLPageState();
}

class _DumpsysLPageState extends State<DumpsysLPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysLService.fetch(deviceId);
    if (mounted) {
      setState(() {
