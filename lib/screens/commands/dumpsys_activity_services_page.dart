...
class DumpsysActivityServicesPage extends StatefulWidget {
  const DumpsysActivityServicesPage({super.key});

  @override
  State<DumpsysActivityServicesPage> createState() => _DumpsysActivityServicesPageState();
}

class _DumpsysActivityServicesPageState extends State<DumpsysActivityServicesPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysActivityServicesService.fetch(deviceId);
    if (mounted) {
      setState(() {
