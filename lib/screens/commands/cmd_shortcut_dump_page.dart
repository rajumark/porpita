...
class CmdShortcutDumpPage extends StatefulWidget {
  const CmdShortcutDumpPage({super.key});

  @override
  State<CmdShortcutDumpPage> createState() => _CmdShortcutDumpPageState();
}

class _CmdShortcutDumpPageState extends State<CmdShortcutDumpPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await CmdShortcutDumpService.fetch(deviceId);
    if (mounted) {
      setState(() {
