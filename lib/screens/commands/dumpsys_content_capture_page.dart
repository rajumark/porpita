...
class DumpsysContentCapturePage extends StatefulWidget {
  const DumpsysContentCapturePage({super.key});

  @override
  State<DumpsysContentCapturePage> createState() => _DumpsysContentCapturePageState();
}

class _DumpsysContentCapturePageState extends State<DumpsysContentCapturePage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysContentCaptureService.fetch(deviceId);
    if (mounted) {
      setState(() {
