...
class DumpsysDisplayPage extends StatefulWidget {
  const DumpsysDisplayPage({super.key});

  @override
  State<DumpsysDisplayPage> createState() => _DumpsysDisplayPageState();
}

class _DumpsysDisplayPageState extends State<DumpsysDisplayPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysDisplayService.fetch(deviceId);
    if (mounted) {
      setState(() {
