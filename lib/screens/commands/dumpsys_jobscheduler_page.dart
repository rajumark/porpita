...
class DumpsysJobschedulerPage extends StatefulWidget {
  const DumpsysJobschedulerPage({super.key});

  @override
  State<DumpsysJobschedulerPage> createState() => _DumpsysJobschedulerPageState();
}

class _DumpsysJobschedulerPageState extends State<DumpsysJobschedulerPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysJobschedulerService.fetch(deviceId);
    if (mounted) {
      setState(() {
