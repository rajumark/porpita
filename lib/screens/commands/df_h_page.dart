...
class DfHPage extends StatefulWidget {
  const DfHPage({super.key});

  @override
  State<DfHPage> createState() => _DfHPageState();
}

class _DfHPageState extends State<DfHPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DfHService.fetch(deviceId);
    if (mounted) {
      setState(() {
