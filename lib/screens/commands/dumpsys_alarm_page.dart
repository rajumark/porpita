...
class DumpsysAlarmPage extends StatefulWidget {
  const DumpsysAlarmPage({super.key});

  @override
  State<DumpsysAlarmPage> createState() => _DumpsysAlarmPageState();
}

class _DumpsysAlarmPageState extends State<DumpsysAlarmPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysAlarmService.fetch(deviceId);
    if (mounted) {
      setState(() {
