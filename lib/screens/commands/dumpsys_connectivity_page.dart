...
class DumpsysConnectivityPage extends StatefulWidget {
  const DumpsysConnectivityPage({super.key});

  @override
  State<DumpsysConnectivityPage> createState() => _DumpsysConnectivityPageState();
}

class _DumpsysConnectivityPageState extends State<DumpsysConnectivityPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysConnectivityService.fetch(deviceId);
    if (mounted) {
      setState(() {
