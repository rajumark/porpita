...
class DmesgPage extends StatefulWidget {
  const DmesgPage({super.key});

  @override
  State<DmesgPage> createState() => _DmesgPageState();
}

class _DmesgPageState extends State<DmesgPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DmesgService.fetch(deviceId);
    if (mounted) {
      setState(() {
