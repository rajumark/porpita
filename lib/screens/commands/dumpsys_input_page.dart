...
class DumpsysInputPage extends StatefulWidget {
  const DumpsysInputPage({super.key});

  @override
  State<DumpsysInputPage> createState() => _DumpsysInputPageState();
}

class _DumpsysInputPageState extends State<DumpsysInputPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysInputService.fetch(deviceId);
    if (mounted) {
      setState(() {
