...
class DumpsysShortcutPage extends StatefulWidget {
  const DumpsysShortcutPage({super.key});

  @override
  State<DumpsysShortcutPage> createState() => _DumpsysShortcutPageState();
}

class _DumpsysShortcutPageState extends State<DumpsysShortcutPage> {
  String _output = '';
  bool _loading = false;
  String? _deviceId;
  ...
  Future<void> _fetch(String deviceId) async {
    ...
    final out = await DumpsysShortcutService.fetch(deviceId);
    if (mounted) {
      setState(() {
