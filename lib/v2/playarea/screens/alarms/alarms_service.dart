...
class AlarmsService {
  static Future<String> fetchRaw(String deviceId) async {
    return AdbExecService.run(deviceId, ['dumpsys', 'alarm']);
  }
  ...
  static Future<(AlarmSettings, List<AlarmEntry>)> fetchAlarms(String deviceId) async {
    ...
    if (raw.isEmpty) {
      return (
        AlarmSettings(version: '', minFuturity: '', minInterval: '', maxInterval: '', minWindow: '', maxAlarmsPerUid: '', raw: {}),
        <AlarmEntry>[],
      );
    }
  ...
  static AlarmSettings _parseSettings(String output) {
  ...
  static List<AlarmEntry> _parseAlarms(String output) {
...
final blockLines = <String>[];
    int endLine = i + 1;
      ...
      final alarmIndex = int.tryParse(headerMatch[2]!) ?? 0;
      final entry = _parseAlarmBlock(headerMatch[1]!, alarmIndex, trimmed, blockLines);
      entries.add(entry);
      i = endLine;
    }
  ...
  static AlarmEntry _parseAlarmBlock(String typeLabel, int alarmIndex, String headerLine, List<String> blockLines) {
    final raw = <String, String>{};
