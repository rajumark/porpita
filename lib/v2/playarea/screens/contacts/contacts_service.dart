...
class ContactsService {
  ...
  static Future<List<Map<String, String>>> fetchRawRows(
    String deviceId, {
    String uri = _contactsUri,
    List<String> columns = kContactColumns,
  }) async {
    return ContentQueryParser.query(
      deviceId: deviceId,
      uri: uri,
  ...
  static Future<ContactDetails> fetchContactDetails(
    ...
    try {
      ...
      final dataRows = await ContentQueryParser.query(
        deviceId: deviceId,
        uri: _dataUri,
        knownColumns: kContactDataColumns,
      );

      final filtered = dataRows
          .where((r) => r['contact_id'] == contactId)
          .map(ContactDataEntry.fromMap)
          .toList();
  ...
  static Future<ContactDetails> fetchContactDetailsByNumber(
    ...
    for (final candidate in variations) {
      final where = "mimetype='vnd.android.cursor.item/phone_v2' AND data1='$candidate'";
      try {
